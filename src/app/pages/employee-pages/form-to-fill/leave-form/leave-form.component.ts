...
this.levelName=localDataObject[0]?.designation[0].jobLevel,
    ...
    this.initLeaveForm();
    this.services.getAllLeaveType().subscribe((succ)=>{
      console.log(succ)
  ...
  // onDateChange(): void {
  //   if (this.LeaveForm.get('startDate').valid && this.LeaveForm.get('endDate').valid) {
  ...
  //       numberOfDaysRequested: daysExcludingSundays
  //     });
  //   }
  // }
...
// method for upload file
  ...
  onSelect(event:any){
    switch (event.target.name){
      case "leaveType":
        if (this.LeaveForm.controls['leaveType'].value === '1') {
